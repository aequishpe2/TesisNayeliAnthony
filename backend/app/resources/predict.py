...
class Predict(Resource):
    def post(self):
        try:
            ...
            # Convertir valores numpy a float para serialización JSON
            prediccion_dict = {k: float(v) for k, v in prediccion_dict.items()}

            # Guardar en la base de datos
            nueva_prediccion = Prediccion(
                texto=texto, 
                prediccion=json.dumps(prediccion_dict)
            )
